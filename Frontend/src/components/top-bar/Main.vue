    ...
    <div class="h-full flex items-center">
      ...
      <div class="intro-x dropdown w-8 h-8">
        ...
        <div class="dropdown-menu w-56">
          <div
            class="dropdown-menu__content box bg-theme-11 dark:bg-dark-6 text-white drd"
          >
...
export default defineComponent({
  ...
  methods: {
    logout() {
      ...
      let headers = {
        apiKey: "life-is-forty-two",
        anotherHeader: "some-header-valuee"
      };
      headers["x-refresh-token"] = info.tokens.refreshToken.token;
      return new Promise((resolve, reject) => {
        axios
          .post(process.env.VUE_APP_ROOT_API + "/v1/auth/logout", "data", {
            headers
          })
          .then(response => {
            if (VueCookies.get("info")) VueCookies.remove("info");
            if (localStorage.getItem("info")) localStorage.removeItem("info");
            resolve(response);
            router.push("/");
